...
export async function GET(req: NextRequest,
    ...
    try {
        ...
        const { data: authData } = await supabaseAdmin.auth.admin.getUserById(student_id);
        const fullName = authData.user?.user_metadata?.full_name;
        const email = authData.user?.email ?? undefined;

        const student: StudentClassroomProfile = {
            id: student_id,
            classroom_id: classroom_id,
            accepted: studentData.accepted,
            invited_at: studentData.invited_at,
            joined_at: studentData.joined_at,
            name: (typeof fullName === "string" ? fullName.trim() : "") || studentData.profiles?.[0]?.nickname?.trim() || `Student ${student_id.slice(0, 6)}`,
            email: email,
            profile: {
