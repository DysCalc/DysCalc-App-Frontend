  ...
  const handleAcceptInvite = async () => {
    ...
    try {
      ...
      if (!profile) {
        ...
        router.push(`/setup?role=STUDENT&class_id=${classId}`);
        return;
      }
    ...
    <div className="flex min-h-screen flex-col items-center justify-center bg-[#f5f5f0] px-4">
      <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden p-8 text-center border border-gray-100">
        ...
        <p className="text-gray-600 mb-6 text-lg">
          {invitedBy ? <span className="font-semibold text-gray-800">{invitedBy}</span> : "An educator"}{" "}
          invited you to join <span className="font-bold text-[#29A177]">{className || "their class"}</span>!
        </p>

        <div className="space-y-4">
          {loading ? (
            <div className="flex justify-center py-4">
              <Loader2 className="w-8 h-8 animate-spin text-[#29A177]" />
            </div>
          ) : user ? (
            <div className="space-y-4">
              ...
              <button
                ...
                className="w-full bg-[#29A177] text-white font-bold py-3 px-4 rounded-xl shadow-md hover:bg-[#208360] hover:shadow-lg transition-all duration-200 disabled:opacity-70 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
